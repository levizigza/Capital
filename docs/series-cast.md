# Series cast — Cashwell Capital

Bring the illustrated series leads into Fortune Archipelago **one by one**.  
Iconic path freeze still holds: no map widening; Piggy + Coin Bag own the signature loop.

## Roles (do not blur)

| Role | Who | Owns |
|------|-----|------|
| **Series leads** | Cashwell · Cashmere · Pedro · Fernanda · Bao · Jade · Sultan · Dahlia · Mansa · Kandake · Moneybagg (11 of ~13) | Brand faces, Memory Courtyard presence, style of living money |
| **Harbor Keeper** | Piggy Penny | First meet, quiet homecoming, Return verbs |
| **Path buddy** | Coin Bag | Pointing, carpet seat, next-good-action |

## Cast pipeline (per character)

1. Sheet art → `moneyCast.ts` id + tagline + locked look  
2. Harbor plaza slot (never Piggy’s fountain) **or** island ally later  
3. Talk tip that names organ / Plinth without stealing Piggy beats  
4. Distinctive mesh accents (hat, glyph, coat) — procedural first  
5. Hide or mute during `isPiggyPresenceBeat`  
6. Story-bible one-liner  

## Roster progress

| # | Name | Status |
|---|------|--------|
| 1 | **Cashwell** | In Harbor (Memory Courtyard terrace) |
| 2 | **Cashmere Couture** | In Harbor (Memory Courtyard terrace) |
| 3 | **Peso Pedro** | In Harbor (Memory Courtyard terrace) |
| 4 | **Fortuna Fernanda** | In Harbor (Memory Courtyard terrace) |
| 5 | **Billionaire Bao** | In Harbor (Memory Courtyard terrace) |
| 6 | **Jade Fortune** | In Harbor (Memory Courtyard terrace) |
| 7 | **Sultan Stacks** | In Harbor (Memory Courtyard terrace) |
| 8 | **Dinar Dahlia** | In Harbor (Memory Courtyard terrace) |
| 9 | **Mansa Moneybaggs** | In Harbor (Memory Courtyard terrace) |
| 10 | **Kandake Kash** | In Harbor (Memory Courtyard terrace) |
| 11 | **Moneybagg Bro** | In Harbor (Memory Courtyard terrace) |
| 12–13 | TBD | Wait for sheets |

When in doubt: the series leads tip hats and celebrate — **Piggy keeps the verbs**.
