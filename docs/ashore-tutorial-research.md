...
## 6. Implementation priority (when we build next)

1. **Shrink Ashore to ≤5 prove-it chambers** (delete triangle glossary pages).  
2. **Dock chamber in 3D pad** — board Cove with Carpet language, not a tap card alone.  
3. **Strengthen first real loop** — Cove Take cinema + carpet home CTA + spectacle clarity (already iconic-path).  
4. **Structure enter as first Soft Beat/arcade teach** — inside Jar after (or during) Cove, not in Ashore.  
5. **Share only post-spectacle** — remove Ashore share freeze.  
6. **Playtest cold** → rewrite copy to one-sentence budgets.
